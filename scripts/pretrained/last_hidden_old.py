...
if trained == 'True':
    ...
    results_folder = join(ROOT, 'data', 'pretrained', 'last_hidden_trained_5reps')
else:
    print('\nnInstantiating randomly initialized model')
    exec('model = ' + arch + '(pretrained=False)')
    results_folder = join(ROOT, 'data', 'pretrained', 'last_hidden_untrained_5reps')
...
if save:
    np.save(join(results_folder, arch + '_ID_training_sample_mix5.npy'), ID)